...
pub trait DynamoDBItemOp<TD: TableDefinition>: DynamoDBItem<TD> {
    ...
    /// [`HasAttribute<A>`] for every key of the IndexDefinition to confirm the
    /// type participates in that index.
    ///
    /// Also, note that this method will fail if the index does not contain only items
    ...
    ///
    /// `I` must be an [`IndexDefinition`] for `TD`, and `Self` must implement
    /// [`HasAttribute<A>`] for every key of the IndexDefinition. The key
